...
class Build
{
    ...
    public static function module($module = '', $list = [], $namespace = 'app', $suffix = false)
    {
        ...
        foreach ($list as $path => $file) {
            $modulePath = APP_PATH . $module . DIRECTORY_SEPARATOR;
            if ('__dir__' == $path) {
                // 生成子目录
                foreach ($file as $dir) {
                ...
                foreach ($file as $val) {
                    $val      = trim($val);
                    $filename = $modulePath . $path . DIRECTORY_SEPARATOR . $val . ($suffix ? ucfirst($path) : '') . EXT;
    ...
    {
        $filename = APP_PATH . ($module ? $module . DIRECTORY_SEPARATOR : '') . 'controller' . DIRECTORY_SEPARATOR . 'Index' . ($suffix ? 'Controller' : '') . EXT;
        if (!is_file($filename)) {
            $content = file_get_contents(THINK_PATH . 'tpl' . DIRECTORY_SEPARATOR . 'default_index.tpl');
    ...
    {
        $filename = CONF_PATH . ($module ? $module . DIRECTORY_SEPARATOR : '') . 'config.php';
        if (!is_file($filename)) {
            file_put_contents($filename, "<?php\n//配置文件\nreturn [\n\n];");
        }
        $filename = APP_PATH . ($module ? $module . DIRECTORY_SEPARATOR : '') . 'common.php';
        if (!is_file($filename)) {
            file_put_contents($filename, "<?php\n;");
        }
    }
