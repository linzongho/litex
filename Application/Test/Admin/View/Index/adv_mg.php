<?
	...
	function showForm($action) {
		global $PHP_SELF,$MYSQL,$SiteImgAdv,$file_table_name,$file_title,$dbpre,$start_catg,$page,$style,$keyword,$param2,$id,$catg_code,$title,$url,$simage,$addtime;
		$id = $_REQUEST['id'];
		if($action == "modify"){
...
<div class="formbody">
	<ul class="forminfo">
		...
		<li><label><?=$file_title?>链接：</label><input name="url" id="url" type="text" class="dfinput" value="<?=$url?>" style="width:500px;" /></li>	
		<?
			if ($simage<>"") $str1="<a href='".$SiteImgAdv.$simage."' rel='exgroup' title='".$title."'><img src='images/bimg.gif' width='20' height='20' title='预览' border='0' /></a>&nbsp;&nbsp;<a href='?action=delimg&id=".$id."&start_catg=".$start_catg."&style=".$style."&keyword=".$keyword."&param2=".$param2."&page=".$page."'>删除该图</a>"; else $str1="<font color=red>无图</font>";
		?>
...
<?
	function showFormTo($action,$info) {
		global $PHP_SELF,$MYSQL,$SiteImgAdv,$file_table_name,$file_title,$dbpre,$start_catg,$page,$style,$keyword,$param2,$cinfo,$catg_code;
?>
<script type="text/javascript" language="javascript">
<!--
function trim(str){
...
<body>
    ...
    <div class="rightinfo">
		...
		if($action == "list"){
			...
			$sql = "select id,title,catg_code,addtime,simage from ".$dbpre.$file_table_name." where 1=1 $where order by id desc";
		...
		<tbody>
			<? 
				while($row = mysql_fetch_array($query)){
					...
					if (trim($row["simage"])<>""){
						$imgstr = '<a href="'.$SiteImgAdv.$row["simage"].'" rel="exgroup" title="'.$row["title"].'"><img src="'.$SiteImgAdv.$row["simage"].'" border="0" width="80"  height="60" /></a>';
					}
			?>
			<tr>
			<td><input type="checkbox" name="cinfo[]" value="<?=$row["id"]?>" /></td>
		...
		<ul class="imglist">
    		<? 
				while($row = mysql_fetch_array($query)){
					...
					if (trim($row["simage"])<>""){
						$imgstr = '<a href="'.$SiteImgAdv.$row["simage"].'" rel="exgroup" title="'.$row["title"].'"><img src="'.$SiteImgAdv.$row["simage"].'" border="0" width="168"  height="127" /></a>';
					}
			?>
			<li>
				<span><?=$imgstr?></span>
...
if($action == "add"){
    if(isset($_REQUEST['add'])){
		...
		if(!empty($_FILES['simage']['name'])){
			...
			copy($_FILES['simage']['tmp_name'],$SiteImgAdv.$simage_str);
        }
		
 		if(@$MYSQL->query("insert into ".$dbpre.$file_table_name." (catg_code,title,url,simage,addtime) values ('$catg_code','$title','$url','$simage_str','$addtime')",$db)){
        	$id = $MYSQL->insert_id();
...
if($action == "modify") {
	...
	if(isset($_REQUEST['modify'])){
		...
		if(!empty($_FILES['simage']['name'])){
			...
			if($row["simage"])
				@unlink($SiteImgAdv.$row["simage"]);
        }
		
		$where = "";
		if(!empty($simage_str)) $where .=",simage = '".$simage_str."'";
        $qry_update = "update ".$dbpre.$file_table_name." set catg_code='$catg_code',title='$title',url='".$url."',addtime='$addtime' ".$where." where id=$id";
