<?
	...
	function showForm($action) {
		global $PHP_SELF,$MYSQL,$SiteImgProduct,$file_table_name,$file_title,$dbpre,$start_catg,$page,$style,$keyword,$param2,$id,$catg_code,$title,$model,$content,$simage,$simage2,$simage3,$simage4,$simage5,$addtime;
		$id = $_REQUEST['id'];
		if($action == "modify"){
...
<div class="formbody">
	<ul class="forminfo">
		...
		<li><label><?=$file_title?>名称：<b>*</b></label><input name="title" id="title" type="text" class="dfinput" value="<?=$title?>" style="width:500px;" /></li>
		<li><label><?=$file_title?>型号：</label><input name="model" id="model" type="text" class="dfinput" value="<?=$model?>" /></li>	
		<?
			if ($simage<>"") $str1="<a href='".$SiteImgProduct.$simage."' rel='exgroup' title='".$title."'><img src='images/bimg.gif' width='20' height='20' title='预览' border='0' /></a>&nbsp;&nbsp;<a href='?action=delimg&id=".$id."&start_catg=".$start_catg."&style=".$style."&keyword=".$keyword."&param2=".$param2."&page=".$page."'>删除该图</a>"; else $str1="<font color=red>无图</font>";
			
			if ($simage2<>"") $str2="<a href='".$SiteImgProduct.$simage2."' rel='exgroup' title='".$title."'><img src='images/bimg.gif' width='20' height='20' title='预览' border='0' /></a>&nbsp;&nbsp;<a href='?action=delimg2&id=".$id."&start_catg=".$start_catg."&style=".$style."&keyword=".$keyword."&param2=".$param2."&page=".$page."'>删除该图</a>"; else $str2="<font color=red>无图</font>";
			
			if ($simage3<>"") $str3="<a href='".$SiteImgProduct.$simage3."' rel='exgroup' title='".$title."'><img src='images/bimg.gif' width='20' height='20' title='预览' border='0' /></a>&nbsp;&nbsp;<a href='?action=delimg3&id=".$id."&start_catg=".$start_catg."&style=".$style."&keyword=".$keyword."&param2=".$param2."&page=".$page."'>删除该图</a>"; else $str3="<font color=red>无图</font>";
			
			if ($simage4<>"") $str4="<a href='".$SiteImgProduct.$simage4."' rel='exgroup' title='".$title."'><img src='images/bimg.gif' width='20' height='20' title='预览' border='0' /></a>&nbsp;&nbsp;<a href='?action=delimg4&id=".$id."&start_catg=".$start_catg."&style=".$style."&keyword=".$keyword."&param2=".$param2."&page=".$page."'>删除该图</a>"; else $str4="<font color=red>无图</font>";
			
			if ($simage5<>"") $str5="<a href='".$SiteImgProduct.$simage5."' rel='exgroup' title='".$title."'><img src='images/bimg.gif' width='20' height='20' title='预览' border='0' /></a>&nbsp;&nbsp;<a href='?action=delimg5&id=".$id."&start_catg=".$start_catg."&style=".$style."&keyword=".$keyword."&param2=".$param2."&page=".$page."'>删除该图</a>"; else $str5="<font color=red>无图</font>";
		?>
		<li><label>上传图片：</label><input name="simage" type="file" class="dfinput" value="" /><i><?=$str1?></i></li>
		<li><label>上传图片2：</label><input name="simage2" type="file" class="dfinput" value="" /><i><?=$str2?></i></li>
		<li><label>上传图片3：</label><input name="simage3" type="file" class="dfinput" value="" /><i><?=$str3?></i></li>
		<li><label>上传图片4：</label><input name="simage4" type="file" class="dfinput" value="" /><i><?=$str4?></i></li>
		<li><label>上传图片5：</label><input name="simage5" type="file" class="dfinput" value="" /><i><?=$str5?></i></li>	
...
<?
	function showFormTo($action,$info) {
		global $PHP_SELF,$MYSQL,$SiteImgProduct,$file_table_name,$file_title,$dbpre,$start_catg,$page,$style,$keyword,$param2,$cinfo,$catg_code;
?>
<script type="text/javascript" language="javascript">
<!--
function trim(str){
...
<body>
    ...
    <div class="rightinfo">
		...
		if($action == "list"){
			...
			if ($keyword<>"") $where.=" and  (title like '%".$keyword."%' or model like '%".$keyword."%')";
			if ($param2<>"") {
				$where.=" and (catg_code like '".$param2."-%' or catg_code = '".$param2."')";
			}else{
				$where.=" and (catg_code like '".$start_catg."-%' or catg_code = '".$start_catg."')";
			}
			$sql = "select id,title,model,catg_code,addtime,simage from ".$dbpre.$file_table_name." where 1=1 $where order by id desc";
		...
		<form action="?start_catg=<?=$start_catg?>&style=<?=$style?>" method="post"><ul class="seachform">
			...
			<li><label><?=$file_title?>名称</label><input name="keyword" type="text" class="scinput" value="<?=$keyword?>" style="width:250px;" /></li>
		...
		<tbody>
			<? 
				while($row = mysql_fetch_array($query)){
					...
					if (trim($row["simage"])<>""){
						$imgstr = '<a href="'.$SiteImgProduct.$row["simage"].'" rel="exgroup" title="'.$row["title"].'"><img src="'.$SiteImgProduct.$row["simage"].'" border="0" width="80"  height="60" /></a>';
					}
			?>
			<tr>
			<td><input type="checkbox" name="cinfo[]" value="<?=$row["id"]?>" /></td>
		...
		<ul class="imglist">
    		<? 
				while($row = mysql_fetch_array($query)){
					...
					if (trim($row["simage"])<>""){
						$imgstr = '<a href="'.$SiteImgProduct.$row["simage"].'" rel="exgroup" title="'.$row["title"].' '.$row["model"].'"><img src="'.$SiteImgProduct.$row["simage"].'" border="0" width="168"  height="127" /></a>';
					}
			?>
			<li>
				<span><?=$imgstr?></span>
...
if($action == "add"){
    if(isset($_REQUEST['add'])){
		...
		if(!empty($_FILES['simage']['name'])){
			...
			copy($_FILES['simage']['tmp_name'],$SiteImgProduct.$simage_str);
        }
		
		if(!empty($_FILES['simage2']['name'])){
        	if (!check_file_type($_FILES['simage2']['tmp_name'],$_FILES['simage2']['name'], $allow_pic_types)){
				ErrBack("图片格式错误");
			}
			$ext = strtolower(substr($_FILES['simage2']['name'], strrpos($_FILES['simage2']['name'], '.')));
			$simage2_str=md5(uniqid(microtime(),1)).$ext;
			copy($_FILES['simage2']['tmp_name'],$SiteImgProduct.$simage2_str);
        }
		
		if(!empty($_FILES['simage3']['name'])){
        	if (!check_file_type($_FILES['simage3']['tmp_name'],$_FILES['simage3']['name'], $allow_pic_types)){
				ErrBack("图片格式错误");
			}
			$ext = strtolower(substr($_FILES['simage3']['name'], strrpos($_FILES['simage3']['name'], '.')));
			$simage3_str=md5(uniqid(microtime(),1)).$ext;
			copy($_FILES['simage3']['tmp_name'],$SiteImgProduct.$simage3_str);
        }
		
		if(!empty($_FILES['simage4']['name'])){
        	if (!check_file_type($_FILES['simage4']['tmp_name'],$_FILES['simage4']['name'], $allow_pic_types)){
				ErrBack("图片格式错误");
			}
			$ext = strtolower(substr($_FILES['simage4']['name'], strrpos($_FILES['simage4']['name'], '.')));
			$simage4_str=md5(uniqid(microtime(),1)).$ext;
			copy($_FILES['simage4']['tmp_name'],$SiteImgProduct.$simage4_str);
        }
		
		if(!empty($_FILES['simage5']['name'])){
        	if (!check_file_type($_FILES['simage5']['tmp_name'],$_FILES['simage5']['name'], $allow_pic_types)){
				ErrBack("图片格式错误");
			}
			$ext = strtolower(substr($_FILES['simage5']['name'], strrpos($_FILES['simage5']['name'], '.')));
			$simage5_str=md5(uniqid(microtime(),1)).$ext;
			copy($_FILES['simage5']['tmp_name'],$SiteImgProduct.$simage5_str);
        }
		
 		if(@$MYSQL->query("insert into ".$dbpre.$file_table_name." (catg_code,title,model,content,simage,simage2,simage3,simage4,simage5,addtime) values ('$catg_code','$title','$model','$content','$simage_str','$simage2_str','$simage3_str','$simage4_str','$simage5_str','$addtime')",$db)){
        	$id = $MYSQL->insert_id();
...
if($action == "modify") {
	...
	if(isset($_REQUEST['modify'])){
    	...
    	if(empty($title)) $err = "必须填写".$file_title."名称";
        if(isset($err)) {
        	echo "<br><center><font color=#ff0000 ><b>错误：".$err."</b></font></center><br><br>";
            showForm("modify");
            exit;
		...
		if(!empty($_FILES['simage']['name'])){
			...
			if($row["simage"])
				@unlink($SiteImgProduct.$row["simage"]);
        }
		...
		if(!empty($_FILES['simage2']['name'])){
			...
			$img = @$MYSQL->query("select simage2 from ".$dbpre.$file_table_name." where id=".$id);
			$row = mysql_fetch_array($img);
			if($row["simage2"])
				@unlink($SiteImgProduct.$row["simage2"]);
        }
		...
		if(!empty($_FILES['simage3']['name'])){
			...
			$img = @$MYSQL->query("select simage3 from ".$dbpre.$file_table_name." where id=".$id);
			$row = mysql_fetch_array($img);
			if($row["simage3"])
				@unlink($SiteImgProduct.$row["simage3"]);
        }
		...
		if(!empty($_FILES['simage4']['name'])){
			...
			$img = @$MYSQL->query("select simage4 from ".$dbpre.$file_table_name." where id=".$id);
			$row = mysql_fetch_array($img);
			if($row["simage4"])
				@unlink($SiteImgProduct.$row["simage4"]);
        }
		...
		if(!empty($_FILES['simage5']['name'])){
			...
			$img = @$MYSQL->query("select simage5 from ".$dbpre.$file_table_name." where id=".$id);
			$row = mysql_fetch_array($img);
			if($row["simage5"])
				@unlink($SiteImgProduct.$row["simage5"]);
        }
		
		$where = "";
		if(!empty($simage_str)) $where .=",simage = '".$simage_str."'";
		if(!empty($simage2_str)) $where .=",simage2 = '".$simage2_str."'";
		if(!empty($simage3_str)) $where .=",simage3 = '".$simage3_str."'";
		if(!empty($simage4_str)) $where .=",simage4 = '".$simage4_str."'";
		if(!empty($simage5_str)) $where .=",simage5 = '".$simage5_str."'";
        $qry_update = "update ".$dbpre.$file_table_name." set catg_code='$catg_code',title='$title',model='$model',content='".$content."',addtime='$addtime' ".$where." where id=$id";
...
if($action == "AllDel") {
	...
	if (is_array($cinfo)) {
		foreach($cinfo as $value){
			$where.= " or id='".$value."'";
			$img = @$MYSQL->query("select simage,simage2,simage3,simage4,simage5 from ".$dbpre.$file_table_name." where id='".$value."'");
			$row = mysql_fetch_array($img);
			if($row["simage"])
				@unlink($SiteImgProduct.$row["simage"]);
			if($row["simage2"])
...
if($action == "remove") {
	$id = $_REQUEST['id'];
	$img = @$MYSQL->query("select simage,simage2,simage3,simage4,simage5 from ".$dbpre.$file_table_name." where id=".$id);
	$row = mysql_fetch_array($img);
	if($row["simage"])
		@unlink($SiteImgProduct.$row["simage"]);
	if($row["simage2"])
...
if($action == "delimg2") {
	...
	if($row["simage2"])
		@unlink($SiteImgProduct.$row["simage2"]);
   	if($MYSQL->query("update ".$dbpre.$file_table_name." set simage2='' where id=".$id))
     	ErrTo("图片已删除",$file_table_name."_mg.php?action=modify&id=".$id."&start_catg=".$start_catg."&style=".$style."&keyword=".$keyword."&page=".$page."&param2=".$param2);
   	else
     	ErrBack("出现错误，图片无法删除。");
}
if($action == "delimg3") {
	...
	if($row["simage3"])
		@unlink($SiteImgProduct.$row["simage3"]);
   	if($MYSQL->query("update ".$dbpre.$file_table_name." set simage3='' where id=".$id))
     	ErrTo("图片已删除",$file_table_name."_mg.php?action=modify&id=".$id."&start_catg=".$start_catg."&style=".$style."&keyword=".$keyword."&page=".$page."&param2=".$param2);
   	else
     	ErrBack("出现错误，图片无法删除。");
}
if($action == "delimg4") {
	...
	if($row["simage4"])
		@unlink($SiteImgProduct.$row["simage4"]);
   	if($MYSQL->query("update ".$dbpre.$file_table_name." set simage4='' where id=".$id))
     	ErrTo("图片已删除",$file_table_name."_mg.php?action=modify&id=".$id."&start_catg=".$start_catg."&style=".$style."&keyword=".$keyword."&page=".$page."&param2=".$param2);
   	else
     	ErrBack("出现错误，图片无法删除。");
}
if($action == "delimg5") {
	...
	if($row["simage5"])
		@unlink($SiteImgProduct.$row["simage5"]);
   	if($MYSQL->query("update ".$dbpre.$file_table_name." set simage5='' where id=".$id))
     	ErrTo("图片已删除",$file_table_name."_mg.php?action=modify&id=".$id."&start_catg=".$start_catg."&style=".$style."&keyword=".$keyword."&page=".$page."&param2=".$param2);
   	else
     	ErrBack("出现错误，图片无法删除。");
}
